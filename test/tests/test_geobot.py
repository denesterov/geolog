...
async def test_idling(mock_location_start_factory, mock_location_update_factory, mock_context):
    ...
    for point in track[1]:
        await geobot.cmd_message(mock_location_update_factory(up1, point), mock_context, final_point=point is track[1][-1])
    ...
    test_utils.help_test_gpx_data(sessions[0].id, track, 173.0 + 148.0, 120.0)
